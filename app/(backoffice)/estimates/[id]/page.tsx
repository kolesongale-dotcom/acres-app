...
export default async function EstimateBuilderPage({
  ...
  const initial: BuilderState = {
    ...
    cabinetSets: estimate.cabinetSets.map((c) => ({ id: c.id, name: c.name, doorCount: c.doorCount, drawerCount: c.drawerCount, frameCount: c.frameCount, coats: c.coats, primerCoats: c.primerCoats, primerProduct: c.primerProduct, paintProduct: c.paintProduct, paintId: c.paintId, primerId: c.primerId, materials: parseMats(c.materials), sortOrder: c.sortOrder })),
    deckAreas: estimate.deckAreas.map((d) => ({ id: d.id, name: d.name, floorLength: d.floorLength, floorWidth: d.floorWidth, includeRailing: d.includeRailing, railingLinFt: d.railingLinFt, stepCount: d.stepCount, includeLattice: d.includeLattice, latticeSqFt: d.latticeSqFt, coats: d.coats, powerWashCost: d.powerWashCost, woodReplCost: d.woodReplCost, floorStainId: d.floorStainId, railStainId: d.railStainId, primerPaintId: d.primerPaintId, primerCoats: d.primerCoats, primerSqftAdjust: d.primerSqftAdjust, materials: parseMats(d.materials), sortOrder: d.sortOrder })),
    exteriorHouses: estimate.exteriorHouses.map((h) => ({ id: h.id, name: h.name, sidingLength: h.sidingLength, sidingWidth: h.sidingWidth, sidingHeight: h.sidingHeight, sidingSqftAdjust: h.sidingSqftAdjust, sidingMaterial: h.sidingMaterial, coats: h.coats, paintProduct: h.paintProduct, paintId: h.paintId, primerPaintId: h.primerPaintId, primerCoats: h.primerCoats, primerSqftAdjust: h.primerSqftAdjust, materials: parseMats(h.materials), sortOrder: h.sortOrder, deductions: h.deductions.map((d) => ({ label: d.label, width: d.width, height: d.height })), replacements: h.replacements.map((r) => ({ description: r.description, cost: r.cost })) })),
    exteriorDoors: estimate.exteriorDoors.map((d) => ({ id: d.id, name: d.name, count: d.count, width: d.width, height: d.height, paintedSides: d.paintedSides, coats: d.coats, paintProduct: d.paintProduct, paintId: d.paintId, primerPaintId: d.primerPaintId, primerCoats: d.primerCoats, primerSqftAdjust: d.primerSqftAdjust, materials: parseMats(d.materials), sortOrder: d.sortOrder })),
    exteriorShutters: estimate.exteriorShutters.map((s) => ({ id: s.id, name: s.name, story1: s.story1, story2: s.story2, story3: s.story3, customQty: s.customQty, customRate: s.customRate, coats: s.coats, paintProduct: s.paintProduct, paintId: s.paintId, primerPaintId: s.primerPaintId, primerCoats: s.primerCoats, primerSqftAdjust: s.primerSqftAdjust, materials: parseMats(s.materials), sortOrder: s.sortOrder })),
    garageDoors: estimate.garageDoors.map((g) => ({ id: g.id, name: g.name, count: g.count, width: g.width, height: g.height, coats: g.coats, paintProduct: g.paintProduct, paintId: g.paintId, primerPaintId: g.primerPaintId, primerCoats: g.primerCoats, primerSqftAdjust: g.primerSqftAdjust, materials: parseMats(g.materials), sortOrder: g.sortOrder })),
    customAreas: estimate.customAreas.map((c) => ({ id: c.id, label: c.label, measureType: c.measureType, amount: c.amount, rate: c.rate, coats: c.coats, paintProduct: c.paintProduct, paintId: c.paintId, primerPaintId: c.primerPaintId, primerCoats: c.primerCoats, primerSqftAdjust: c.primerSqftAdjust, materials: parseMats(c.materials), sortOrder: c.sortOrder })),
